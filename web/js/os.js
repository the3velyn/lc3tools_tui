// os.js - LC-3 Operating System assembly source
// Ported from src/backend/lc3os.cpp

export const LC3_OS_ASM = `
    .ORIG x0000
...
OS_START
    ...
    ADD R6, R6, #-1
    STR R0, R6, #0
    ; push synthesized PC (x3000) onto system stack
    LD R0, USER_PC
    ADD R6, R6, #-1
    STR R0, R6, #0
    ; enter user mode
...
; ---- TRAP x20: GETC ----
; Read a single character from keyboard into R0 (no echo)
TRAP_GETC
...
; ---- TRAP x21: OUT ----
; Write character in R0 to the display
TRAP_OUT
    ADD R6, R6, #-1
    STR R1, R6, #0          ; save R1
TRAP_OUT_WAIT
...
; ---- TRAP x22: PUTS ----
; Write null-terminated string at address in R0 to the display
TRAP_PUTS
...
; ---- TRAP x23: IN ----
; Prompt for and read a single character into R0, echo to display
TRAP_IN
...
; ---- TRAP x24: PUTSP ----
; Write packed string (2 chars per word) at address in R0
...
TRAP_PUTSP_MSB_0
    ...
    BRz TRAP_PUTSP_DONE
    OUT                       ; otherwise print the high byte
...
; ---- TRAP x25: HALT ----
; Halt execution: print message, then clear bit 15 of MCR
TRAP_HALT
...
; ---- BAD_TRAP handler ----
; Print error message and halt for undefined traps
BAD_TRAP
    LEA R0, BAD_TRAP_MSG
    PUTS
    HALT
